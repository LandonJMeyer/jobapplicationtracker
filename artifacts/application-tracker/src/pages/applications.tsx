...
export default function Applications() {
  ...
  const [categoryFilter, setCategoryFilter] = useState(() => new URLSearchParams(window.location.search).get('filter') || 'all');
  ...
  const visibleApplications = useMemo(() => {
    if (categoryFilter !== 'followups') return applications;
    const now = Date.now();
    return applications.filter((application) => {
      if (!application.followUpAt || ['Rejected', 'Withdrawn', 'Offer'].includes(application.status)) return false;
      return new Date(application.followUpAt).getTime() <= now;
    });
  }, [applications, categoryFilter]);
  const clearFilters = () => { setSearch(''); setStatus('All'); setCategoryFilter('all'); };
  const viewTitle = categoryFilter === 'followups' ? 'Follow-ups due' : status === 'All' ? 'Your pipeline' : `${status} applications`;
  const viewDescription = categoryFilter === 'followups' ? 'Every application that needs a next touch.' : 'Every opportunity, one calm place to keep moving.';
  ...
  return (
    <div className="mx-auto max-w-[1440px]">
       <div className="rise-in flex flex-col justify-between gap-5 md:flex-row md:items-end"><div><p className="font-data text-[10px] uppercase tracking-[.18em] text-primary">Application library</p><h1 className="mt-2 font-display text-[clamp(2rem,4vw,3.25rem)] font-bold leading-[1.02] tracking-[-.06em]">{viewTitle}<br /><span className="text-primary">in full.</span></h1><p className="mt-3 text-[13px] text-muted-foreground">{viewDescription}</p></div><Button onClick={() => setDialogOpen(true)} className="w-fit rounded-xl" data-testid="button-create-application"><Plus /> Add application</Button></div>
      <section className="rise-in rise-in-delay-1 mt-8 overflow-hidden rounded-2xl border border-card-border bg-card shadow-[0_8px_30px_rgba(24,43,64,.045)]">
        <div className="flex flex-col gap-3 border-b border-border p-4 md:flex-row"><div className="relative flex-1"><Search className="pointer-events-none absolute left-3 top-2.5 size-4 text-muted-foreground" /><Input type="search" value={search} onChange={(e) => setSearch(e.target.value)} className="h-9 border-border bg-background pl-9 text-xs" placeholder="Search company or role" data-testid="input-search-applications" />{search && <button onClick={() => setSearch('')} className="absolute right-2 top-2 rounded p-0.5 text-muted-foreground hover:text-foreground" data-testid="button-clear-search"><X className="size-3.5" /></button>}</div><div className="flex items-center gap-2"><div className="flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-[.12em] text-muted-foreground"><SlidersHorizontal className="size-3.5" /> Filter</div><div className="flex gap-1 overflow-x-auto">{filters.map((filter) => <button key={filter} onClick={() => { setStatus(filter); setCategoryFilter('all'); }} className={`whitespace-nowrap rounded-lg px-2.5 py-2 text-[11px] font-bold transition-colors ${status === filter ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:bg-secondary hover:text-foreground'}`} data-testid={`button-filter-${filter.toLowerCase()}`}>{filter}</button>)}</div></div></div>
         <div className="flex items-center justify-between px-5 py-3"><div className="flex items-center gap-2 text-[11px] text-muted-foreground"><ListFilter className="size-3.5" /><span><strong className="text-foreground">{visibleApplications.length}</strong> {visibleApplications.length === 1 ? 'application' : 'applications'}</span></div>{(status !== 'All' || categoryFilter !== 'all' || search) && <button className="text-[11px] font-bold text-primary hover:underline" onClick={clearFilters} data-testid="button-clear-filters">Clear filters</button>}</div>
         {isLoading ? <TableSkeleton /> : isError ? <LoadError onRetry={() => refetch()} /> : <ApplicationTable applications={visibleApplications} onDelete={setDeleteTarget} />}
      </section>
